...
public class MovementManager : MonoBehaviour
{
    ...
    public void DrawPath( Tile target)
    {
        ...
        path.SetPosition(0, new Vector3(target.transform.position.x, target.transform.position.y, -0.5f));
        Tile tile = target;
            ...
            for (int i = 0; i < target.dist; i++)
            {
                ...
                path.positionCount++;
                tile = tile.parent;
                path.SetPosition(i +1 , new Vector3(tile.transform.position.x, tile.transform.position.y, -0.5f));
